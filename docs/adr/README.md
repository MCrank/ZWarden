...
| [0043](./0043-graceful-restart-is-a-best-effort-agent-side-broadcast-before-the-safe-stop.md) | Graceful restart is a best-effort, Agent-side `servermsg` broadcast + bounded descending countdown that wraps the F15 safe stop; default across every restart path (F15/F17/F22); an RCON failure never blocks the restart; additive `RestartServer.Plan`, no new permission | #114 |
| [0044](./0044-workshop-search-is-an-optional-per-tenant-operator-supplied-key.md) | Workshop search is an optional, per-tenant, operator-supplied Steam Web API key (keyless by default; a standard `dev/apikey` key, not a publisher key); the key is AEAD-encrypted, write-only, redacted, `Tenant.Manage`-gated + audited, and control-plane-only | #110 |
| [0045](./0045-recreate-removes-a-stopped-container-by-its-canonical-name-only.md) | Recreate removes a stopped container by its canonical `srv-<uuid>` name only: the proxy admits `DELETE` for that path shape alone (amends 0008), and the Agent re-asserts ownership, requires it stopped, and never forces or drops volumes | #229 |
