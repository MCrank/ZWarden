...
- **New packages** in `Directory.Packages.props` (exact pins, ADR 0002): `Aspire.Hosting.AppHost`, `Aspire.Hosting.PostgreSQL`, `Aspire.Hosting.Testing` (13.5.3). **Correction (PR-1):** the AppHost and the Aspire test project carry **no `packages.lock.json`** (`RestorePackagesWithLockFile=false`) — with `AspireUseCliBundle=false` the SDK restores host-RID-specific dashboard/DCP packages, so a Windows-generated lock fails locked-mode restore on the linux CI runner (NU1004) and a cross-platform lock is not achievable. Exact pins in `Directory.Packages.props` still stop versions floating; production projects keep their locks (ADR 0031 decision 9; the earlier "commit lockfiles" line in Slice 1/DoD is superseded).
