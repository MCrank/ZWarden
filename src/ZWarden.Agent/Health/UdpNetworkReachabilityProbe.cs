...
/// <summary>
/// The default <see cref="INetworkReachabilityProbe"/> (F16). It sends a zero-length datagram to the host
/// loopback port and waits briefly: a closed UDP port yields an ICMP port-unreachable that the OS surfaces as a
...
/// exists. Best-effort by construction (ADR-noted in the F16 plan); exercised in the integration tier, not
/// offline.
/// </summary>
public sealed class UdpNetworkReachabilityProbe : INetworkReachabilityProbe
{
    ...
    /// <inheritdoc />
    public async Task<bool?> IsUdpPortReachableAsync(int port, CancellationToken cancellationToken)
    {
